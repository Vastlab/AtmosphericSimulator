...
class Simulator(nn.Module):
    ...
    def forward(self, img):
        ...
        MVx = torch.fft.ifftn((self.S_half*torch.randn(self.img_size,self.img_size,device=self.device)).permute(1,2,0),dim=(0,1)).abs() # Changed with fft addition
        MVy = torch.fft.ifftn((self.S_half*torch.randn(self.img_size,self.img_size,device=self.device)).permute(1,2,0),dim=(0,1)).abs() # Changed with fft addition
